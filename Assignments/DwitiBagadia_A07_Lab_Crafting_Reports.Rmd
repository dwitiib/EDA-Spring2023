...
summary.PM25 <- epa.data %>% 
  drop_na(PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(mean.PM2.5 = mean(PM2.5)) %>% 
  arrange(mean.PM2.5)

#Compute mean ozone concentration for each county, sorted by value
summary.Ozone <- epa.data %>% 
  drop_na(Ozone) %>% 
  group_by(COUNTY) %>% 
  summarize(
...
```{r tables, echo=FALSE}
#Using `kable` function to produce the 2 tables

kable(summary.PM25, format = "simple", row.names = TRUE, 
      col.names = c("County", "$\\mu g/m^3$"), caption = "Mean Particulates (2.5mm)")

kable(summary.Ozone, format = "simple", row.names = TRUE, 
...
```{r plot, warning=FALSE, echo=FALSE, message=FALSE, fig.cap="Distribution of Ozone Levels", fig.align='left', fig.height=4, fig.width=6}
#loading libraries
library(gridExtra)
library(latex2exp)

#creating plots finally 
Ozone18 <- epa.data %>% drop_na(Ozone) %>% filter(Year == 2018)
Ozone19 <- epa.data %>% drop_na(Ozone) %>% filter(Year == 2019)
par(mar = c(4,4,4,4))
Ozone19.plot = ggplot(Ozone19) + 
  geom_boxplot(aes(x = Month, y = Ozone)) + 
  labs(x = "Month(2019)", y = TeX("Ozone($\\mu g/m^3$)"))

Ozone18.plot = ggplot(Ozone18) + 
  geom_boxplot(aes(x = Month, y = Ozone)) + 
  labs(x = "Month(2018)", y = TeX("Ozone($\\mu g/m^3$)"))

grid.arrange(Ozone18.plot, Ozone19.plot, ncol = 2)
...
### Git Repository
https://github.com/dwitiib/EDA-Spring2023.git
